    ...
    case MobID::kBoulder:
        {
            ...
            for (uint32_t i = 1; i < sides; ++i)
            {
                ...
                ctx.line_to(cosf(angle) * radius + gen.binext() * deflection,
                            sinf(angle) * radius + gen.binext() * deflection);
            }
            ctx.close_path();
            ctx.fill();
            ctx.stroke();
...
#define draw_leg(angle) \
    ...
    case MobID::kEllipse:
        ...
        break;
    case MobID::kSegment:
        ctx.scale(scale);                          // 应用缩放
        ctx.set_stroke(0xffcfbc55);                // 描边颜色（可按需修改）
        ctx.set_line_width(0.15 * 40);             // 线宽，可改为根据长度自适应
        ctx.round_line_cap();                      // 圆头端点
        ctx.round_line_join();                     // 圆角连接（此处其实不影响）
        ctx.begin_path();
