...
static void _alloc_drops(Simulation* sim, std::vector<PetalID::T>& success_drops_id,
                         std::vector<RarityID::T>& success_drops_rarity, float x, float y)
{
#ifdef DEBUG
...
void entity_on_death(Simulation* sim, Entity const& ent)
{
    ...
    if (ent.has_component(kFlower) && sim->ent_alive(ent.get_parent()))
    {
        ...
        EntityID killer_id = sim->ent_exists(ent.last_damaged_by)
                                 ? sim->get_ent(ent.last_damaged_by).base_entity
                                 : NULL_ENTITY;
    ...
    if (ent.has_component(kMob))
    {
            ...
            frand() < DIGGER_SPAWN_CHANCE)
        {
            ...
            alloc_mob(sim, MobID::kDigger, ent.get_mob_rarity(), ent.get_x(), ent.get_y(), team);
        }
    }
